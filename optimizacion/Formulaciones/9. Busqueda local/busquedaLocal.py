...
def miRadar(x, d, n=8):
    ...
    if isinstance(x, tuple):
        if len(x) == 2:
            # Se guardan los valores del parámetro t en los que se evalua la ecuación paramétrica del circulo en una lista: [0, π/4, 2π/4, 3π/4, π, 5π/4, 6π/4, 7π/4, 2π].
            t = [i / (n / 2) * pi for i in range(n)]
            # Se evalua la ecuación paramétrica del círculo en cada uno de los puntos de la lista t y se guardan en una lista.
            puntos = [(x[0] + d * cos(i), x[1] + d * sin(i)) for i in t]
            # Retorna los puntos del radar centrado en x con radio d
            return puntos
        else:
...
def busquedaLocal(f, restricciones, radar, x0, d0):
    ...
    while not Termine:  # Mientras que no se haya terminado se repite el la búsqueda
        ...
        candidatos = radar(
            xStar, d0
        )  # Se usa la función radar para obtener los puntos a evaluar
        for p in candidatos:  # Se evaluan todos los puntos en el radar
            if f(*p) < f(
                *xStar
            ):  # Si el punto a evaluar tiene valor en la función objetivo menor al punto actual, es candidato a ser el nuevo mejor punto y se evalua factibilidad.
                Factible = True  # Se asume que el punto p es factible
                for (
                    r
                ) in (
                    restricciones
                ):  # Se evalua que el punto p cumpla todas las restricciones
                ...
                if (
                    Factible
                ):  # Si el punto p es factible (i.e., Factible==True), el nuevo mejor punto es p
...
def busquedaLocalGrafica(f, restricciones, radar, x0, d0, xlim, ylim):
    ...
    ax = mg.metodoGrafico(f, restricciones, grad=None, xlim=xlim, ylim=ylim)
    h = (xlim[1] - xlim[0]) / 100
